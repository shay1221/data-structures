...
class AVLTree(object):
	...
	def insert(self, key, val):
		...
		y = z.parent
		while(y is not None):
			bf = AVLTree.get_Height(y.left) -  AVLTree.get_Height(y.right)
	...
	def rotate(self, node):
		ret = 0
		bf =AVLTree.get_Height(node.left) - AVLTree.get_Height(node.right)
		if(bf == 2):
			#b = AVLNode.get_Height(node.get_left().get_left()) - AVLNode.get_Height(node.get_left().get_right())
			b = self.compute_bf(node.left)
			if(b == 1):
	...
	def right_rotation(self, node : AVLNode):
		...
		y = node
		node.NodeUpdating()
		node.left.NodeUpdating()
		node.right.NodeUpdating()
		if n.parent is not None:
			n.parent.NodeUpdating()
